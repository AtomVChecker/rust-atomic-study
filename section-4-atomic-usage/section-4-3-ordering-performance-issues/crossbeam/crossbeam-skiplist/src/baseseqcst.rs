...
impl<K, V> Node<K, V> {
    ...
    fn height(&self) -> usize {
        (self.refs_and_height.load(Ordering::SeqCst) & HEIGHT_MASK) + 1 // Relaxed
    }
    ...
    fn is_removed(&self) -> bool {
        let tag = unsafe {
            ...
            // `epoch::unprotected()` in this situation.
            self.tower[0]
                .load(Ordering::SeqCst, epoch::unprotected()) // Relaxed
                .tag()
        };
        tag == 1
    }
    ...
    unsafe fn try_increment(&self) -> bool {
        let mut refs_and_height = self.refs_and_height.load(Ordering::SeqCst); // Relaxed
    ...
    #[inline]
    unsafe fn decrement_with_pin<F>(&self, parent: &SkipListSeqCst<K, V>, pin: F)
    where
        F: FnOnce() -> Guard,
    {
        if self
...
unsafe impl<K: Send + Sync, V: Send + Sync> Send for SkipListSeqCst<K, V> {}
unsafe impl<K: Send + Sync, V: Send + Sync> Sync for SkipListSeqCst<K, V> {}

impl<K, V> SkipListSeqCst<K, V> {
    /// Returns a new, empty skip list.
    pub fn new(collector: Collector) -> SkipListSeqCst<K, V> {
        SkipListSeqCst {
            head: Head::new(),
            collector,
    ...
    pub fn len(&self) -> usize {
        let len = self.hot_data.len.load(Ordering::SeqCst); // Relaxed
    ...
    K: Ord,
{
    ...
    fn random_height(&self) -> usize {
        ...
        let mut num = self.hot_data.seed.load(Ordering::SeqCst); // Relaxed
        num ^= num << 13;
        num ^= num >> 17;
        num ^= num << 5;
        self.hot_data.seed.store(num, Ordering::SeqCst); // Relaxed
        ...
        let mut max_height = self.hot_data.max_height.load(Ordering::SeqCst);// Relaxed
        ...
        Q: Ord + ?Sized,
    {
        unsafe {
            'search: loop {
                ...
                let mut level = self.hot_data.max_height.load(Ordering::SeqCst); // Relaxed

                // Fast loop to skip empty tower levels.
                while level >= 1
                    && self.head[level - 1]
                        .load(Ordering::SeqCst, guard) // Relaxed
                        .is_null()
                {
                    level -= 1;
                }
        ...
        unsafe {
            ...
            loop {
                ...
                if replace {
                    ...
                    if r.mark_tower() {
                        self.hot_data.len.fetch_sub(1, Ordering::SeqCst); // Relaxed
                    }
                } else {
            ...
            self.hot_data.len.fetch_add(1, Ordering::SeqCst); // Relaxed

            loop {
                // Set the lowest successor of `n` to `search.right[0]`.
                n.tower[0].store(search.right[0], Ordering::SeqCst); // Relaxed
            ...
            'build: for level in 1..height {
                loop {
                    ...
                    n.refs_and_height
                        .fetch_add(1 << HEIGHT_BITS, Ordering::SeqCst); // Relaxed
                    ...
                    n.refs_and_height
                        .fetch_sub(1 << HEIGHT_BITS, Ordering::SeqCst); // Relaxed
...
impl<K, V> Drop for SkipListSeqCst<K, V> {
    fn drop(&mut self) {
        unsafe {
...
impl<K, V> fmt::Debug for SkipListSeqCst<K, V>
where
    K: Ord + fmt::Debug,
    V: fmt::Debug,
{
...
impl<K, V> IntoIterator for SkipListSeqCst<K, V> {
    type Item = (K, V);
    ...
    fn into_iter(self) -> IntoIter<K, V> {
        unsafe {
            ...
            for level in 0..MAX_HEIGHT {
                self.head[level].store(Shared::null(), Ordering::SeqCst); // Relaxed
            }
...
impl<'a: 'g, 'g, K: 'a, V: 'a> Entry<'a, 'g, K, V> {
    ...
    pub fn skiplistSeqCst(&self) -> &'a SkipListSeqCst<K, V> {
        self.parent
    }
    ...
    V: Send + 'static,
{
    ...
    pub fn remove(&self) -> bool {
        ...
        if self.node.mark_tower() {
            ...
            self.parent.hot_data.len.fetch_sub(1, Ordering::SeqCst); // Relaxed
    ...
    fn drop(&mut self) {
        ...
        while !self.node.is_null() {
            unsafe {
                // Unprotected loads are okay because this function is the only one currently using
                // the skip list.
                let next = (*self.node).tower[0].load(Ordering::SeqCst, epoch::unprotected()); // Relaxed
