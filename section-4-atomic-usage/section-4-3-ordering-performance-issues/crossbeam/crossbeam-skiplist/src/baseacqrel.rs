...
impl<K, V> Node<K, V> {
    ...
    fn height(&self) -> usize {
        (self.refs_and_height.load(Ordering::Acquire) & HEIGHT_MASK) + 1 // Relaxed
    }
    ...
    fn mark_tower(&self) -> bool {
        ...
        for level in (0..height).rev() {
            let tag = unsafe {
                ...
                self.tower[level]
                    .fetch_or(1, Ordering::Release, epoch::unprotected()) // SeqCst
                    .tag()
            };
    ...
    fn is_removed(&self) -> bool {
        let tag = unsafe {
            ...
            // `epoch::unprotected()` in this situation.
            self.tower[0]
                .load(Ordering::Acquire, epoch::unprotected()) // Relaxed
                .tag()
        };
        tag == 1
    }
    ...
    unsafe fn try_increment(&self) -> bool {
        let mut refs_and_height = self.refs_and_height.load(Ordering::Acquire); // Relaxed
    ...
    #[inline]
    unsafe fn decrement_with_pin<F>(&self, parent: &SkipListAcqRel<K, V>, pin: F)
    where
        F: FnOnce() -> Guard,
    {
        if self
...
unsafe impl<K: Send + Sync, V: Send + Sync> Send for SkipListAcqRel<K, V> {}
unsafe impl<K: Send + Sync, V: Send + Sync> Sync for SkipListAcqRel<K, V> {}

impl<K, V> SkipListAcqRel<K, V> {
    /// Returns a new, empty skip list.
    pub fn new(collector: Collector) -> SkipListAcqRel<K, V> {
        SkipListAcqRel {
            head: Head::new(),
            collector,
    ...
    pub fn len(&self) -> usize {
        let len = self.hot_data.len.load(Ordering::Acquire); // Relaxed
    ...
    K: Ord,
{
    ...
    fn random_height(&self) -> usize {
        ...
        let mut num = self.hot_data.seed.load(Ordering::Acquire); // Relaxed
        num ^= num << 13;
        num ^= num >> 17;
        num ^= num << 5;
        self.hot_data.seed.store(num, Ordering::Release); // Relaxed
        ...
        let mut max_height = self.hot_data.max_height.load(Ordering::Acquire);// Relaxed
        ...
        Q: Ord + ?Sized,
    {
        unsafe {
            'search: loop {
                ...
                let mut level = self.hot_data.max_height.load(Ordering::Acquire); // Relaxed

                // Fast loop to skip empty tower levels.
                while level >= 1
                    && self.head[level - 1]
                        .load(Ordering::Acquire, guard) // Relaxed
                        .is_null()
                {
                    level -= 1;
                }
        ...
        unsafe {
            ...
            loop {
                ...
                if replace {
                    ...
                    if r.mark_tower() {
                        self.hot_data.len.fetch_sub(1, Ordering::AcqRel); // Relaxed
                    }
                } else {
            ...
            self.hot_data.len.fetch_add(1, Ordering::AcqRel); // Relaxed

            loop {
                // Set the lowest successor of `n` to `search.right[0]`.
                n.tower[0].store(search.right[0], Ordering::Release); // Relaxed
            ...
            'build: for level in 1..height {
                loop {
                    ...
                    let next = n.tower[level].load(Ordering::Acquire, guard); // SeqCst
                    ...
                    // TODO(Amanieu): can we use release ordering here?
                    if n.tower[level]
                        .compare_exchange(next, succ, Ordering::AcqRel, Ordering::Acquire, guard) // SeqCst
                        .is_err()
                    {
                        break 'build;
                    }
                    ...
                    n.refs_and_height
                        .fetch_add(1 << HEIGHT_BITS, Ordering::AcqRel); // Relaxed
                    ...
                    n.refs_and_height
                        .fetch_sub(1 << HEIGHT_BITS, Ordering::AcqRel); // Relaxed
            ...
            // level.
            // TODO(Amanieu): can we use relaxed ordering here?
            if n.tower[height - 1].load(Ordering::Acquire, guard).tag() == 1 { // SeqCst
            ...
            loop {
                ...
                if n.mark_tower() {
                    ...
                    for level in (0..n.height()).rev() {
                        // TODO(Amanieu): can we use relaxed ordering here?
                        let succ = n.tower[level].load(Ordering::Acquire, guard).with_tag(0); // SeqCst
...
impl<K, V> Drop for SkipListAcqRel<K, V> {
    fn drop(&mut self) {
        unsafe {
...
impl<K, V> fmt::Debug for SkipListAcqRel<K, V>
where
    K: Ord + fmt::Debug,
    V: fmt::Debug,
{
...
impl<K, V> IntoIterator for SkipListAcqRel<K, V> {
    type Item = (K, V);
    ...
    fn into_iter(self) -> IntoIter<K, V> {
        unsafe {
            ...
            for level in 0..MAX_HEIGHT {
                self.head[level].store(Shared::null(), Ordering::Release); // Relaxed
            }
...
impl<'a: 'g, 'g, K: 'a, V: 'a> Entry<'a, 'g, K, V> {
    ...
    pub fn skiplistAcqRel(&self) -> &'a SkipListAcqRel<K, V> {
        self.parent
    }
    ...
    V: Send + 'static,
{
    ...
    pub fn remove(&self) -> bool {
        ...
        if self.node.mark_tower() {
            ...
            self.parent.hot_data.len.fetch_sub(1, Ordering::AcqRel); // Relaxed
    ...
    fn drop(&mut self) {
        ...
        while !self.node.is_null() {
            unsafe {
                // Unprotected loads are okay because this function is the only one currently using
                // the skip list.
                let next = (*self.node).tower[0].load(Ordering::Acquire, epoch::unprotected()); // Relaxed
