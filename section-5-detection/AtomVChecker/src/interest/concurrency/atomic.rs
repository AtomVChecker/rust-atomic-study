...
impl PartialOrd for AtomicOrd {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        ...
        match (*self, *other) {
            ...
            | (AcqRel, Relaxed)
            | (AcqRel, Acquire)
            | (AcqRel, Release) => Some(Ordering::Greater),
            (_, SeqCst)
            | (Relaxed, Release)
            | (Relaxed, Acquire)
            | (Relaxed, AcqRel)
            | (Acquire, AcqRel)
            | (Release, AcqRel) => Some(Ordering::Less),
        }
    }
}
...
impl AtomicInstructions {
    pub fn from_instance<'tcx>(instance: Instance<'tcx>, tcx: TyCtxt<'tcx>) -> Option<Self> {
        ...
        if path.ends_with("compare_exchange")
            || path.ends_with("compare_exchange_weak")
            || path.ends_with("fetch_update")
        {
...
impl<'a, 'tcx> AtomicCollector<'a, 'tcx> {
    pub fn new(tcx: TyCtxt<'tcx>, instance_id: InstanceId, instance: &'a Instance<'tcx>) -> Self {
        Self {
            tcx,
            instance_id,
            instance,
    ...
    pub fn analyze(&mut self, callgraph: &CallGraph<'tcx>) {
        let callers: Vec<InstanceId> = callgraph
            .graph
            .neighbors_directed(self.instance_id, Incoming)
            .collect();
        for caller in callers {
            ...
            for callsite in callsites.unwrap() {
                ...
                {
                    let atomic_operate: Option<AtomicInstructions> =
                        AtomicInstructions::from_instance(*self.instance, self.tcx);
                    ...
                    let source_info = self
                        ...
                        .sess
                        .source_map()
                        .span_to_diagnostic_string(*fn_span);
                    ...
                    if !ordering_type.is_empty() {
                        let atomic_collertor = AtomicInfo::new(
                            atomic_arg,
                            influence_value,
                            atomic_operate,
                            caller,
...
impl<'tcx> AtomPart<'tcx> {
        ...
        for (atomic, interival) in &self.partner {
            ...
            if let Some(ptses) = points_to_map.get(&node) {
                for pts_node in ptses {
                    if let ConstraintNode::Alloc(place_ref) | ConstraintNode::Place(place_ref) =
                        pts_node
                    {
    ...
    pub fn infer_interival(&mut self, callgraph: &CallGraph<'tcx>) {
        ...
        let index = vars_and_temps
            .iter()
            .position(|&x| x == atomic_place_local)
            .unwrap_or(0);
        ...
        match self.atom_info.atomic_operate.unwrap() {
            AtomicInstructions::CompareExchange => {
                ...
                let atomic_node = ConstraintNode::Place(
                    Place::from(self.atom_info.atomic_value[1].local).as_ref(),
                );
                let atomic_pts = points_to_map.get(&atomic_node).unwrap();
                interim_val = atomic_pts
                    .iter()
                    .filter_map(|atomic_node| match atomic_node {
                        ConstraintNode::Alloc(place) | ConstraintNode::Place(place)
                            if place.local.index()
                                < self.atom_info.atomic_value[1].local.index()
                ...
                let deps = datadep::all_data_dep_on(
                    ...
                    &data_deps,
                    callgraph,
                    self.atom_info.caller_instance,
                    body,
                    self.tcx,
                );
                for dep in deps {
            ...
            AtomicInstructions::Store => {
                let points_to_map = alias_analysis
                    ...
                    .clone();
                let atomic_node = ConstraintNode::Place(
                    Place::from(self.atom_info.atomic_value[0].local).as_ref(),
                );
                ...
                if let Some(atomic_pts) = atomic_pts {
                    ...
                    interim_val = atomic_pts
                        ...
                        .filter_map(|atomic_node| match atomic_node {
                            ConstraintNode::Alloc(place) | ConstraintNode::Place(place)
                                if place.local.index()
                                    < self.atom_info.atomic_value[0].local.index() =>
                            {
                                Some(place.local)
                            }
                            _ => None,
